...
function scene:createScene( event )   
 	...
 	local function beginListener (event)
 		if (event.phase == "began") then
 			audio.dispose(sound)
			sound = nil
			storyboard.gotoScene("level2", { effect = "fade", time = 400, params = {curScoree = score}})
 		end
 		return true
 	end
...
function displayObjects()
	bg = display.newImage ("background.png", width/2, height/2)
 	bg.alpha = .5
 	bar = display.newImage("slab.png", width, height)
 	bar2 = display.newImage("slab.png", width/2+100, height)
 	pig = stateAlter(width*.90, height, height*.9)  
 	pig2 = stateAlter(width*.55+100, height,height*.6)
