...
function scene:createScene( event )   
 	...
 	local function beginListener (event)
 		if (event.phase == "began") then
 			audio.dispose(sound)
			sound = nil
			storyboard.gotoScene("level3", { effect = "fade", time = 400, params = {curScoree = score}})
 		end
 		return true
 	end
...
function displayObjects()
	bg = display.newImage ("background.png", width*.3, height*.3)
 	bg.alpha = .5
 	bar = display.newImage("slab.png", width/2+150, height)
 	bar1 = display.newImage("slab1.png", width-30, height-50)
 	bar2 = display.newImage("slab.png", width, height-70)
 	pig = stateAlter(width*.90, height, height*.9)  
 	pig2 = stateAlter(width*.55+50, height,height*.6)
